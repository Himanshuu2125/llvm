...
def create_right_pane(parent, app):
    ...
    app.tab_passes = ctk.CTkButton(app.config_container, text="Checkbox Pass Configuration", command=lambda: app.toggle_config_mode('passes'),
    ...
    app.tab_json = ctk.CTkButton(app.config_container, text="Custom JSON Configuration", command=lambda: app.toggle_config_mode('json'),
    ...
    app.start_button = ctk.CTkButton(right_pane, text="Start Obfuscation", command=app.start_obfuscation_placeholder, font=ctk.CTkFont(family='Helvetica', size=15, weight='bold'))
    ...
    ctk.CTkLabel(right_pane, text="Obfuscation Logs", font=ctk.CTkFont(family='Helvetica', size=16, weight='bold')).grid(row=4, column=0, sticky="w", padx=15, pady=(5, 5))
    
    app.log_text = ctk.CTkTextbox(right_pane, height=15, font=('Consolas', 10), activate_scrollbars=True, corner_radius=8, fg_color="#0a0a0a", text_color="#00ff00")
